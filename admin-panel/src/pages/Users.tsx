...
export default function Users() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const result = await apiRequest<User[]>(`/api/users?query=${encodeURIComponent(query)}`);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest('/api/users/update', 'POST', { 
        user_id: editingUser.user_id,
        name: editingUser.name,
        username: editingUser.username,
        coins: editingUser.coins,
        title: editingUser.title,
        is_banned: editingUser.is_banned ? 1 : 0, // Согласовано со схемой API
              ...
              <div style={{ padding: '1rem', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', marginTop: '0.5rem' }}>
                ...
                <select 
                  ...
                  onChange={e => {
                    ...
                    if (val === 'none') {
                      setEditingUser({ ...editingUser, is_banned: false, ban_until: null, ban_reason: '' });
                    } else if (val === 'permanent') {
                      setEditingUser({ ...editingUser, is_banned: true, ban_until: null });
                    } else {
                      // Custom timeouts based on offset added in options
                      setEditingUser({ ...editingUser, is_banned: false, ban_until: parseFloat(val) });
                    }
                  }}
