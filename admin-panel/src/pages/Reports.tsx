...
export default function Reports() {
  ...
  const handleResolve = async (id: number, status: string) => {
    try {
      await fetch('http://127.0.0.1:8000/api/reports/status', {
        method: 'POST',
            ...
            {reports.map(r => (
              <div key={r.id} style={{ 
                ...
                <div className="flex-between" style={{ marginBottom: '1rem' }}>
                  ...
                  <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>{r.created_at}</span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem', fontSize: '0.875rem' }}>
                  <div>
                    ...
                    <div style={{ fontWeight: 500 }}>{r.sender_name} <code>({r.sender_id})</code></div>
                  </div>
                  {r.target_id && (
                    <div>
