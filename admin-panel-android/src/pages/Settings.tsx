...
import { Settings as SettingsIcon, Wrench, Save, RefreshCw, Globe } from 'lucide-react';
import { api } from '../api';
...
export default function Settings() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to fetch settings. Make sure the API URL is correct.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save global settings to server only if we loaded them
      if (Object.keys(settings).length > 0) {
        await api.updateSettings(settings);
      }

      alert('API Connection settings saved successfully!');
      window.location.reload(); // Перезагружаем страницу, чтобы применить новый URL
    } catch (err) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '2rem', gridColumn: '1 / -1' }}>
          <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Globe size={24} color="var(--accent-color)" /> Connection Settings
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>API Base URL</label>
              <input
                type="text"
                value={apiUrl}
        ...
        <div className="glass-panel" style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div>
              <div className="flex-between" style={{ marginBottom: '0.5rem' }}>
                <label style={{ fontWeight: 500 }}>Max Games per User</label>
